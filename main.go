	...
	informer := cache.NewSharedIndexInformer(
		...
		&cache.ListWatch{
			ListFunc: func(options meta_v1.ListOptions) (runtime.Object, error) {
				...
				pods, err := client.CoreV1().Pods(meta_v1.NamespaceDefault).List(options)
				return pods, err
			},
			WatchFunc: func(options meta_v1.ListOptions) (watch.Interface, error) {
				// watch all of the pods (core resource) in the default namespace
				watchFunc, err := client.CoreV1().Pods(meta_v1.NamespaceDefault).Watch(options)
				return watchFunc, err
			},
		},
